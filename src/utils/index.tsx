import { Icon } from "@chakra-ui/react";

export const InstanceIcon = ({ color }: any) => {
  return (
    <Icon fontSize="24px" color="red.500">
      <svg
        viewBox="0 0 28 28"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        data-sentry-element="svg"
        data-sentry-source-file="Instances.tsx"
      >
        <path
          d="M0 8C0 3.58172 3.58172 0 8 0H20C24.4183 0 28 3.58172 28 8V20C28 24.4183 24.4183 28 20 28H8C3.58172 28 0 24.4183 0 20V8Z"
          data-sentry-element="path"
          data-sentry-source-file="Instances.tsx"
        ></path>
        <path
          d="M6.08973 9.43296L13.9999 4.86603L21.9101 9.43296V18.5668L13.9999 23.1338L6.08973 18.5668V9.43296Z"
          stroke={color}
          strokeWidth="1.5"
          strokeLinecap="round"
          data-sentry-element="path"
          data-sentry-source-file="Instances.tsx"
        ></path>
        <path
          d="M11.2858 12.433L13.9998 10.866L16.7139 12.433V15.5669L13.9998 17.1339L11.2858 15.5669V12.433Z"
          stroke={color}
          strokeWidth="1.5"
          strokeLinecap="round"
          data-sentry-element="path"
          data-sentry-source-file="Instances.tsx"
        ></path>
      </svg>
    </Icon>
  );
};

export const ClusterIcon = ({ color }: any) => {
  return (
    <Icon fontSize="24px">
      <svg
        viewBox="0 0 28 28"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        data-sentry-element="svg"
      >
        <path
          d="M0 8C0 3.58172 3.58172 0 8 0H20C24.4183 0 28 3.58172 28 8V20C28 24.4183 24.4183 28 20 28H8C3.58172 28 0 24.4183 0 20V8Z"
          data-sentry-element="path"
        ></path>
        <path
          d="M10.42 7.76602L14.0001 5.69903L17.5802 7.76602V11.9L14.0001 13.967L10.42 11.9V7.76602Z"
          stroke={color}
          strokeWidth="1.5"
          strokeLinecap="round"
          data-sentry-element="path"
        ></path>
        <path
          d="M5.41987 16.099L9 14.032L12.5801 16.099V20.233L9 22.3L5.41987 20.233V16.099Z"
          stroke={color}
          strokeWidth="1.5"
          strokeLinecap="round"
          data-sentry-element="path"
        ></path>
        <path
          d="M15.4199 16.099L19 14.032L22.5801 16.099V20.233L19 22.3L15.4199 20.233V16.099Z"
          stroke={color}
          strokeWidth="1.5"
          strokeLinecap="round"
          data-sentry-element="path"
        ></path>
      </svg>
    </Icon>
  );
};

export const ModelsIcon = ({ color }: any) => {
  return (
    <Icon fontSize="24px">
      <svg
        width="28"
        height="28"
        viewBox="0 0 28 28"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        data-sentry-element="svg"
        data-sentry-source-file="Model.tsx"
      >
        <rect
          width="28"
          height="28"
          fill="transparent"
          data-sentry-element="rect"
          data-sentry-source-file="Model.tsx"
        ></rect>
        <rect
          width="1440"
          height="1129"
          transform="translate(-28 -282)"
          fill="transparent"
          data-sentry-element="rect"
          data-sentry-source-file="Model.tsx"
        ></rect>
        <rect
          x="-28"
          y="-282"
          width="236"
          height="1129"
          fill="transparent"
          data-sentry-element="rect"
          data-sentry-source-file="Model.tsx"
        ></rect>
        <rect
          width="28"
          height="28"
          rx="8"
          fill="transparent"
          data-sentry-element="rect"
          data-sentry-source-file="Model.tsx"
        ></rect>
        <rect
          x="4"
          y="4"
          width="20"
          height="20"
          fill="transparent"
          data-sentry-element="rect"
          data-sentry-source-file="Model.tsx"
        ></rect>
        <path
          d="M14 5.25V13.5834L21.9167 18.5834L21.5 9.41668L14 5.25Z"
          fill="#1C1C1C"
          fillOpacity="0.1"
          data-sentry-element="path"
          data-sentry-source-file="Model.tsx"
        ></path>
        <path
          d="M13.875 4.93819C13.9524 4.89354 14.0477 4.89354 14.125 4.93819L21.7853 9.36086C21.8627 9.40551 21.9103 9.48805 21.9103 9.57736V18.4227C21.9103 18.512 21.8627 18.5945 21.7853 18.6392L14.125 23.0619C14.0477 23.1065 13.9524 23.1065 13.875 23.0619L6.21475 18.6392C6.1374 18.5945 6.08975 18.512 6.08975 18.4227V9.57736C6.08975 9.48805 6.1374 9.40551 6.21475 9.36086L13.875 4.93819Z"
          stroke={color}
          strokeWidth="1.5"
          data-sentry-element="path"
          data-sentry-source-file="Model.tsx"
        ></path>
        <path
          d="M14.0001 9.41699V14.0003M14.0001 14.0003L9.41675 16.5003M14.0001 14.0003L18.5834 16.5003"
          stroke={color}
          strokeWidth="1.5"
          strokeLinecap="round"
          data-sentry-element="path"
          data-sentry-source-file="Model.tsx"
        ></path>
        <rect
          x="-3235.5"
          y="-437.5"
          width="7014"
          height="1398"
          stroke="#1C1C1C"
          strokeDasharray="14 14"
          data-sentry-element="rect"
          data-sentry-source-file="Model.tsx"
        ></rect>
      </svg>
    </Icon>
  );
};

export const SettingsIcon = ({ color }: any) => {
  return (
    <Icon fontSize="24px">
      <svg
        viewBox="0 0 28 28"
        focusable="false"
        className="chakra-icon chakra-icon css-ct77ok"
        data-sentry-element="Icon"
        data-sentry-component="SettingIcon"
      >
        <rect
          width="28"
          height="28"
          fill="transparent"
          data-sentry-element="rect"
        ></rect>
        <rect
          width="1440"
          height="1129"
          transform="translate(-28 -562)"
          fill="transparent"
          data-sentry-element="rect"
        ></rect>
        <rect
          x="-28"
          y="-562"
          width="236"
          height="1129"
          fill="transparent"
          data-sentry-element="rect"
        ></rect>
        <rect
          width="28"
          height="28"
          rx="8"
          fill="transparent"
          data-sentry-element="rect"
        ></rect>
        <circle
          cx="13.65"
          cy="13.6498"
          r="3.85"
          fill={color}
          fillOpacity="0.1"
          data-sentry-element="circle"
        ></circle>
        <mask
          id="path-4-inside-1_13_4979"
          fill="white"
          data-sentry-element="mask"
        >
          <path
            d="M22.3347 12.7516C22.2661 12.3609 21.8989 11.9631 21.5181 11.875L21.2334 11.8082C20.5644 11.6004 19.9709 11.1463 19.5935 10.4774C19.2162 9.80499 19.1269 9.04101 19.2916 8.34052L19.3808 8.06589C19.494 7.68568 19.3465 7.16116 19.0481 6.9006C19.0481 6.9006 18.7804 6.66826 18.0257 6.22116C17.2709 5.77758 16.945 5.65437 16.945 5.65437C16.5779 5.5171 16.0633 5.64735 15.7957 5.93955L15.5967 6.15782C15.0889 6.65069 14.4062 6.94992 13.6515 6.94992C12.8968 6.94992 12.2072 6.64721 11.6994 6.15079L11.5073 5.93953C11.2431 5.64735 10.7251 5.51714 10.358 5.65437C10.358 5.65437 10.0286 5.77758 9.27388 6.22113C8.5191 6.67176 8.25496 6.9041 8.25496 6.9041C7.95652 7.16111 7.80898 7.68213 7.9222 8.06586L8.0046 8.34397C8.16583 9.04451 8.08004 9.80499 7.70265 10.4773C7.32527 11.1497 6.72507 11.6075 6.05272 11.8117L5.77827 11.875C5.40089 11.9631 5.03034 12.3574 4.96175 12.7516C4.96175 12.7516 4.90002 13.1037 4.90002 13.9979C4.90002 14.8921 4.96175 15.2441 4.96175 15.2441C5.03034 15.6384 5.39745 16.0327 5.77827 16.1207L6.0459 16.184C6.71829 16.3883 7.32209 16.8459 7.69948 17.5218C8.07686 18.1943 8.16611 18.9582 8.00143 19.6587L7.92255 19.9298C7.80936 20.31 7.9569 20.8346 8.25529 21.0951C8.25529 21.0951 8.52292 21.3274 9.27767 21.7746C10.0324 22.2217 10.3583 22.3413 10.3583 22.3413C10.7254 22.4786 11.24 22.3484 11.5076 22.0562L11.6963 21.8484C12.2074 21.352 12.8936 21.0493 13.6518 21.0493C14.41 21.0493 15.0995 21.3555 15.6073 21.8519L15.796 22.0597C16.0602 22.3518 16.5781 22.4822 16.9453 22.3448C16.9453 22.3448 17.2746 22.2216 18.0294 21.7781C18.7841 21.3309 19.0483 21.0986 19.0483 21.0986C19.3467 20.8416 19.4943 20.3171 19.381 19.9333L19.2986 19.6517C19.1374 18.9547 19.2232 18.1942 19.6006 17.5253C19.978 16.8529 20.5817 16.3917 21.2542 16.1875L21.5218 16.1242C21.8992 16.0361 22.2697 15.6419 22.3383 15.2476C22.3383 15.2476 22.4 14.8956 22.4 14.0013C22.3964 13.1036 22.3347 12.7515 22.3347 12.7515L22.3347 12.7516ZM13.6517 17.5782C11.7271 17.5782 10.1627 15.9764 10.1627 13.9979C10.1627 12.0229 11.7237 10.4211 13.6517 10.4211C15.5763 10.4211 17.1407 12.0228 17.1407 14.0014C17.1372 15.9764 15.5763 17.5782 13.6517 17.5782Z"
            data-sentry-element="path"
          ></path>
        </mask>
        <path
          d="M22.3347 12.7516C22.2661 12.3609 21.8989 11.9631 21.5181 11.875L21.2334 11.8082C20.5644 11.6004 19.9709 11.1463 19.5935 10.4774C19.2162 9.80499 19.1269 9.04101 19.2916 8.34052L19.3808 8.06589C19.494 7.68568 19.3465 7.16116 19.0481 6.9006C19.0481 6.9006 18.7804 6.66826 18.0257 6.22116C17.2709 5.77758 16.945 5.65437 16.945 5.65437C16.5779 5.5171 16.0633 5.64735 15.7957 5.93955L15.5967 6.15782C15.0889 6.65069 14.4062 6.94992 13.6515 6.94992C12.8968 6.94992 12.2072 6.64721 11.6994 6.15079L11.5073 5.93953C11.2431 5.64735 10.7251 5.51714 10.358 5.65437C10.358 5.65437 10.0286 5.77758 9.27388 6.22113C8.5191 6.67176 8.25496 6.9041 8.25496 6.9041C7.95652 7.16111 7.80898 7.68213 7.9222 8.06586L8.0046 8.34397C8.16583 9.04451 8.08004 9.80499 7.70265 10.4773C7.32527 11.1497 6.72507 11.6075 6.05272 11.8117L5.77827 11.875C5.40089 11.9631 5.03034 12.3574 4.96175 12.7516C4.96175 12.7516 4.90002 13.1037 4.90002 13.9979C4.90002 14.8921 4.96175 15.2441 4.96175 15.2441C5.03034 15.6384 5.39745 16.0327 5.77827 16.1207L6.0459 16.184C6.71829 16.3883 7.32209 16.8459 7.69948 17.5218C8.07686 18.1943 8.16611 18.9582 8.00143 19.6587L7.92255 19.9298C7.80936 20.31 7.9569 20.8346 8.25529 21.0951C8.25529 21.0951 8.52292 21.3274 9.27767 21.7746C10.0324 22.2217 10.3583 22.3413 10.3583 22.3413C10.7254 22.4786 11.24 22.3484 11.5076 22.0562L11.6963 21.8484C12.2074 21.352 12.8936 21.0493 13.6518 21.0493C14.41 21.0493 15.0995 21.3555 15.6073 21.8519L15.796 22.0597C16.0602 22.3518 16.5781 22.4822 16.9453 22.3448C16.9453 22.3448 17.2746 22.2216 18.0294 21.7781C18.7841 21.3309 19.0483 21.0986 19.0483 21.0986C19.3467 20.8416 19.4943 20.3171 19.381 19.9333L19.2986 19.6517C19.1374 18.9547 19.2232 18.1942 19.6006 17.5253C19.978 16.8529 20.5817 16.3917 21.2542 16.1875L21.5218 16.1242C21.8992 16.0361 22.2697 15.6419 22.3383 15.2476C22.3383 15.2476 22.4 14.8956 22.4 14.0013C22.3964 13.1036 22.3347 12.7515 22.3347 12.7515L22.3347 12.7516ZM13.6517 17.5782C11.7271 17.5782 10.1627 15.9764 10.1627 13.9979C10.1627 12.0229 11.7237 10.4211 13.6517 10.4211C15.5763 10.4211 17.1407 12.0228 17.1407 14.0014C17.1372 15.9764 15.5763 17.5782 13.6517 17.5782Z"
          stroke={color}
          strokeWidth="3"
          mask="url(#path-4-inside-1_13_4979)"
          data-sentry-element="path"
        ></path>
        <rect
          x="-3235.5"
          y="-717.5"
          width="7014"
          height="1398"
          stroke="currentColor"
          strokeDasharray="14 14"
          data-sentry-element="rect"
        ></rect>
      </svg>
    </Icon>
  );
};
export const AiExplorerIcon = ({ color }: { color?: string }) => {
  return (
    <Icon fontSize="24px">
      <svg
        viewBox="0 0 28 28"
        focusable="false"
        className="chakra-icon chakra-icon css-ct77ok"
        data-sentry-element="Icon"
        data-sentry-source-file="AiExplorer.tsx"
        data-sentry-component="AiExplorerIcon"
      >
        <rect
          width="28"
          height="28"
          fill="transparent"
          data-sentry-element="rect"
          data-sentry-source-file="AiExplorer.tsx"
        ></rect>
        <rect
          width="1440"
          height="1129"
          transform="translate(-28 -338)"
          fill="transparent"
          data-sentry-element="rect"
          data-sentry-source-file="AiExplorer.tsx"
        ></rect>
        <rect
          x="-28"
          y="-338"
          width="236"
          height="1129"
          fill="transparent"
          data-sentry-element="rect"
          data-sentry-source-file="AiExplorer.tsx"
        ></rect>
        <rect
          width="28"
          height="28"
          rx="8"
          fill="transparent"
          data-sentry-element="rect"
          data-sentry-source-file="AiExplorer.tsx"
        ></rect>
        <rect
          width="28"
          height="28"
          rx="8"
          fill="transparent"
          data-sentry-element="rect"
          data-sentry-source-file="AiExplorer.tsx"
        ></rect>
        <rect
          x="4"
          y="4"
          width="20"
          height="20"
          fill="transparent"
          data-sentry-element="rect"
          data-sentry-source-file="AiExplorer.tsx"
        ></rect>
        <path
          d="M22.6603 11.5V9.57736C22.6603 9.2201 22.4697 8.88997 22.1603 8.71134L14.5 4.28868C14.1906 4.11004 13.8094 4.11004 13.5 4.28868L5.83972 8.71134C5.53032 8.88997 5.33972 9.2201 5.33972 9.57736V11.5M22.6603 16.5V18.4227C22.6603 18.78 22.4697 19.1101 22.1603 19.2887L14.5 23.7114C14.1906 23.89 13.8094 23.89 13.5 23.7114L5.83972 19.2887C5.53032 19.1101 5.33972 18.78 5.33972 18.4227V16.5"
          stroke={color}
          strokeWidth="1.5"
          strokeLinecap="round"
          data-sentry-element="path"
          data-sentry-source-file="AiExplorer.tsx"
        ></path>
        <circle
          cx="5.5"
          cy="13.5"
          r="1.75"
          stroke={color}
          strokeWidth="1.5"
          data-sentry-element="circle"
          data-sentry-source-file="AiExplorer.tsx"
        ></circle>
        <circle
          cx="22.5"
          cy="13.5"
          r="1.75"
          stroke={color}
          strokeWidth="1.5"
          data-sentry-element="circle"
          data-sentry-source-file="AiExplorer.tsx"
        ></circle>
        <path
          d="M10.4022 17H9L11.553 10H13.1747L15.7312 17H14.329L12.3922 11.4355H12.3355L10.4022 17ZM10.4482 14.2554H14.2724V15.2739H10.4482V14.2554Z"
          stroke={color}
          data-sentry-element="path"
          data-sentry-source-file="AiExplorer.tsx"
        ></path>
        <path
          d="M18 10V17H16.6863V10H18Z"
          stroke={color}
          data-sentry-element="path"
          data-sentry-source-file="AiExplorer.tsx"
        ></path>
        <rect
          x="-3235.5"
          y="-493.5"
          width="7014"
          height="1398"
          stroke={color}
          strokeDasharray="14 14"
          data-sentry-element="rect"
          data-sentry-source-file="AiExplorer.tsx"
        ></rect>
      </svg>
    </Icon>
  );
};

export const InterfaceIcon = ({ color }: any) => {
  return (
    <Icon fontSize="24px">
      <svg
        width="28"
        height="28"
        viewBox="0 0 28 28"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        data-sentry-element="svg"
        data-sentry-source-file="Inference.tsx"
      >
        <rect
          width="28"
          height="28"
          rx="8"
          data-sentry-element="rect"
          data-sentry-source-file="Inference.tsx"
        ></rect>
        <path
          d="M16.43 6.7002C19.7163 7.75653 22.1 10.8986 22.1 14.6104C22.1 15.6268 21.9213 16.6004 21.5943 17.5002M11.57 6.7002C8.28377 7.75653 5.90002 10.8986 5.90002 14.6104C5.90002 15.6268 6.07874 16.6004 6.40576 17.5002M19.0403 21.1002C17.657 22.2269 15.9053 22.9002 14 22.9002C12.0203 22.9002 10.2064 22.1733 8.79936 20.9661"
          stroke="currentColor"
          strokeWidth="1.5"
          strokeLinecap="round"
          data-sentry-element="path"
          data-sentry-source-file="Inference.tsx"
        ></path>
        <circle
          cx="14"
          cy="6.7"
          r="1.95"
          stroke={color}
          strokeWidth="1.5"
          strokeLinecap="round"
          data-sentry-element="circle"
          data-sentry-source-file="Inference.tsx"
        ></circle>
        <circle
          cx="20.3"
          cy="19.2996"
          r="1.95"
          stroke={color}
          strokeWidth="1.5"
          strokeLinecap="round"
          data-sentry-element="circle"
          data-sentry-source-file="Inference.tsx"
        ></circle>
        <circle
          cx="7.7"
          cy="19.2996"
          r="1.95"
          stroke={color}
          strokeWidth="1.5"
          strokeLinecap="round"
          data-sentry-element="circle"
          data-sentry-source-file="Inference.tsx"
        ></circle>
      </svg>
    </Icon>
  );
};

export const BillingIcon = ({ color }: any) => {
  return (
    <svg
      viewBox="0 0 28 28"
      focusable="false"
      className="chakra-icon chakra-icon css-onkxuv"
      data-sentry-element="Icon"
      data-sentry-component="BillingIcon"
      data-sentry-source-file="Billing.tsx"
    >
      <rect
        width="28"
        height="28"
        fill="transparent"
        data-sentry-element="rect"
        data-sentry-source-file="Billing.tsx"
      ></rect>
      <rect
        width="1440"
        height="900"
        transform="translate(-28 -506)"
        fill="transparent"
        data-sentry-element="rect"
        data-sentry-source-file="Billing.tsx"
      ></rect>
      <rect
        x="-28"
        y="-506"
        width="236"
        height="900"
        fill="transparent"
        data-sentry-element="rect"
        data-sentry-source-file="Billing.tsx"
      ></rect>
      <rect
        width="28"
        height="28"
        rx="8"
        fill="transparent"
        data-sentry-element="rect"
        data-sentry-source-file="Billing.tsx"
      ></rect>
      <path
        d="M10.7 4.3999L22.25 4.7499V18.7499H19.45L19.1 7.1999H10L10.7 4.3999Z"
        fill={color}
        fillOpacity="0.1"
        data-sentry-element="path"
        data-sentry-source-file="Billing.tsx"
      ></path>
      <rect
        x="5.75"
        y="7.1499"
        width="13.5"
        height="16.5"
        rx="2.25"
        stroke={color}
        strokeWidth="1.5"
        data-sentry-element="rect"
        data-sentry-source-file="Billing.tsx"
      ></rect>
      <path
        d="M11.7252 20.3999V10.3999H12.3511V20.3999H11.7252ZM13.5305 13.5991C13.4949 13.258 13.3448 12.9924 13.0802 12.8022C12.8181 12.6121 12.4771 12.5171 12.0573 12.5171C11.7621 12.5171 11.5089 12.5627 11.2977 12.6538C11.0865 12.745 10.9249 12.8687 10.813 13.0249C10.701 13.1812 10.6438 13.3595 10.6412 13.5601C10.6412 13.7267 10.6781 13.8713 10.7519 13.9937C10.8282 14.116 10.9313 14.2202 11.0611 14.3062C11.1908 14.3895 11.3346 14.4598 11.4924 14.5171C11.6501 14.5744 11.8092 14.6226 11.9695 14.6616L12.7023 14.8491C12.9975 14.9194 13.2812 15.0145 13.5534 15.1343C13.8282 15.2541 14.0738 15.4051 14.2901 15.5874C14.5089 15.7697 14.6819 15.9897 14.8092 16.2476C14.9364 16.5054 15 16.8075 15 17.1538C15 17.6226 14.883 18.0353 14.6489 18.3921C14.4148 18.7463 14.0763 19.0236 13.6336 19.2241C13.1934 19.422 12.6603 19.521 12.0344 19.521C11.4262 19.521 10.8982 19.4246 10.4504 19.2319C10.0051 19.0392 9.65649 18.758 9.40458 18.3882C9.15522 18.0184 9.02036 17.5679 9 17.0366H10.3931C10.4135 17.3153 10.4975 17.547 10.645 17.7319C10.7926 17.9168 10.9847 18.0549 11.2214 18.146C11.4606 18.2371 11.7277 18.2827 12.0229 18.2827C12.3308 18.2827 12.6005 18.2358 12.8321 18.1421C13.0662 18.0457 13.2494 17.9129 13.3817 17.7437C13.514 17.5718 13.5814 17.3713 13.584 17.1421C13.5814 16.9338 13.5216 16.7619 13.4046 16.6265C13.2875 16.4884 13.1234 16.3739 12.9122 16.2827C12.7036 16.189 12.4593 16.1056 12.1794 16.0327L11.2901 15.7983C10.6463 15.6291 10.1374 15.3726 9.76336 15.0288C9.39186 14.6825 9.20611 14.2228 9.20611 13.6499C9.20611 13.1785 9.33079 12.7658 9.58015 12.4116C9.83206 12.0575 10.1743 11.7827 10.6069 11.5874C11.0394 11.3895 11.5293 11.2905 12.0763 11.2905C12.631 11.2905 13.117 11.3895 13.5344 11.5874C13.9542 11.7827 14.2837 12.0549 14.5229 12.4038C14.7621 12.7502 14.8855 13.1486 14.8931 13.5991H13.5305Z"
        fill={color}
        data-sentry-element="path"
        data-sentry-source-file="Billing.tsx"
      ></path>
      <path
        d="M10.1 7.00682V6C10.1 4.89543 10.9954 4 12.1 4H20.7C21.8045 4 22.7 4.89543 22.7 6V16.7C22.7 17.8046 21.8045 18.7 20.7 18.7H19.55"
        stroke={color}
        strokeWidth="1.5"
        data-sentry-element="path"
        data-sentry-source-file="Billing.tsx"
      ></path>
      <rect
        x="-3235.5"
        y="-858.5"
        width="16202"
        height="1398"
        stroke={color}
        strokeDasharray="14 14"
        data-sentry-element="rect"
        data-sentry-source-file="Billing.tsx"
      ></rect>
    </svg>
  );
};
export const MenuIcon = ({ color }: any) => {
  return (
    <svg
      viewBox="0 0 24 24"
      focusable="false"
      xmlns="http://www.w3.org/2000/svg"
      data-sentry-element="Icon"
      data-sentry-source-file="NavControl.tsx"
      aria-hidden="true"
      data-sentry-component="NavControlIcon"
    >
      <mask
        id="path-1-inside-1_1230_3584"
        fill="white"
        data-sentry-element="mask"
        data-sentry-source-file="NavControl.tsx"
      >
        <path
          d="M2 5.88235C2 4.84276 2.89543 4 4 4H20C21.1046 4 22 4.84276 22 5.88235V18.1176C22 19.1572 21.1046 20 20 20H4C2.89543 20 2 19.1572 2 18.1176V5.88235Z"
          data-sentry-element="path"
          data-sentry-source-file="NavControl.tsx"
        ></path>
      </mask>
      <path
        d="M5.5 8.51471C5.91421 8.51471 6.25 8.17892 6.25 7.76471C6.25 7.35049 5.91421 7.01471 5.5 7.01471V8.51471ZM5.5 11.3382C5.91421 11.3382 6.25 11.0024 6.25 10.5882C6.25 10.174 5.91421 9.83824 5.5 9.83824V11.3382ZM5.5 14.1618C5.91421 14.1618 6.25 13.826 6.25 13.4118C6.25 12.9976 5.91421 12.6618 5.5 12.6618V14.1618ZM8.25 4.94118V19.0588H9.75V4.94118H8.25ZM3 8.51471H5.5V7.01471H3V8.51471ZM3 11.3382H5.5V9.83824H3V11.3382ZM3 14.1618H5.5V12.6618H3V14.1618ZM4 5.5H20V2.5H4V5.5ZM20.5 5.88235V18.1176H23.5V5.88235H20.5ZM20 18.5H4V21.5H20V18.5ZM3.5 18.1176V5.88235H0.5V18.1176H3.5ZM4 18.5C3.63689 18.5 3.5 18.2444 3.5 18.1176H0.5C0.5 20.07 2.15397 21.5 4 21.5V18.5ZM20.5 18.1176C20.5 18.2444 20.3631 18.5 20 18.5V21.5C21.846 21.5 23.5 20.07 23.5 18.1176H20.5ZM20 5.5C20.3631 5.5 20.5 5.75556 20.5 5.88235H23.5C23.5 3.92996 21.846 2.5 20 2.5V5.5ZM4 2.5C2.15397 2.5 0.5 3.92996 0.5 5.88235H3.5C3.5 5.75556 3.63689 5.5 4 5.5V2.5Z"
        fill={color}
        mask="url(#path-1-inside-1_1230_3584)"
        data-sentry-element="path"
        data-sentry-source-file="NavControl.tsx"
      ></path>
    </svg>
  );
};

export const LogoIcon = () => {
  return (
    <svg
      viewBox="0 0 172 32"
      focusable="false"
      className="chakra-icon css-oosofh"
      data-sentry-element="Icon"
      data-sentry-source-file="LogoFull.tsx"
      data-sentry-component="LogoFullIcon"
    >
      <svg
        width="172"
        height="32"
        viewBox="0 0 172 32"
        fill="none"
        xmlns="http://www.w3.org/2000/svg"
        data-sentry-element="svg"
        data-sentry-source-file="LogoFull.tsx"
      >
        <g
          clipPath="url(#clip0_4_8941)"
          data-sentry-element="g"
          data-sentry-source-file="LogoFull.tsx"
        >
          <path
            d="M13.216 0H11.5V17.162H13.216V12.7L14.08 10.028C14.76 7.926 15.684 5.912 16.836 4.028C17.162 3.494 17.334 2.88 17.334 2.256V0H13.214H13.216Z"
            fill="black"
            data-sentry-element="path"
            data-sentry-source-file="LogoFull.tsx"
          ></path>
          <path
            d="M24.9223 3.2079L23.7083 1.9939L11.5723 14.1299L12.7863 15.3439L15.9423 12.1879L18.4423 10.9099C20.4083 9.9039 22.4863 9.1339 24.6343 8.6159C25.2423 8.4699 25.7983 8.1579 26.2403 7.7159L27.8343 6.1219L24.9223 3.2099V3.2079Z"
            fill="black"
            data-sentry-element="path"
            data-sentry-source-file="LogoFull.tsx"
          ></path>
          <path
            d="M31.9238 13.2919V11.5759H14.7598V13.2919H19.2218L21.8938 14.1559C23.9958 14.8359 26.0098 15.7599 27.8938 16.9119C28.4278 17.2379 29.0418 17.4099 29.6658 17.4099H31.9218V13.2899L31.9238 13.2919Z"
            fill="black"
            data-sentry-element="path"
            data-sentry-source-file="LogoFull.tsx"
          ></path>
          <path
            d="M29.1956 25.3L30.4096 24.086L18.2736 11.95L17.0596 13.164L20.2156 16.32L21.4936 18.82C22.4996 20.786 23.2696 22.864 23.7876 25.012C23.9336 25.62 24.2456 26.176 24.6876 26.618L26.2816 28.212L29.1936 25.3H29.1956Z"
            fill="black"
            data-sentry-element="path"
            data-sentry-source-file="LogoFull.tsx"
          ></path>
          <path
            d="M18.6737 31.9999H20.3897V14.8379H18.6737V19.2999L17.8097 21.9719C17.1297 24.0739 16.2057 26.0879 15.0537 27.9719C14.7277 28.5059 14.5557 29.1199 14.5557 29.7439V31.9999H18.6757H18.6737Z"
            fill="black"
            data-sentry-element="path"
            data-sentry-source-file="LogoFull.tsx"
          ></path>
          <path
            d="M6.68446 29.23L7.89846 30.444L20.0345 18.308L18.8205 17.094L15.6645 20.25L13.1645 21.528C11.1985 22.534 9.12046 23.304 6.97246 23.822C6.36446 23.968 5.80846 24.28 5.36646 24.722L3.77246 26.316L6.68446 29.228V29.23Z"
            fill="black"
            data-sentry-element="path"
            data-sentry-source-file="LogoFull.tsx"
          ></path>
          <path
            d="M0 18.784V20.5H17.162V18.784H12.7L10.028 17.92C7.926 17.24 5.912 16.316 4.028 15.164C3.494 14.838 2.88 14.666 2.256 14.666H0V18.786V18.784Z"
            fill="black"
            data-sentry-element="path"
            data-sentry-source-file="LogoFull.tsx"
          ></path>
          <path
            d="M2.83998 6.78602L1.62598 8.00002L13.762 20.136L14.976 18.922L11.82 15.766L10.542 13.266C9.53598 11.3 8.76598 9.22202 8.24798 7.07402C8.10198 6.46602 7.78998 5.91002 7.34798 5.46802L5.75398 3.87402L2.84198 6.78602H2.83998Z"
            fill="black"
            data-sentry-element="path"
            data-sentry-source-file="LogoFull.tsx"
          ></path>
          <path
            d="M17.81 11.5759H14.114L11.5 14.1899V17.8859L14.114 20.4999H17.81L20.424 17.8859V14.1899L17.81 11.5759Z"
            fill="black"
            data-sentry-element="path"
            data-sentry-source-file="LogoFull.tsx"
          ></path>
          <path
            d="M52.5177 16.3939L46.4277 7.12793H48.9997L53.5937 14.1879L58.1877 7.12793H60.7597L54.6697 16.3939V25.7899H52.5177V16.3939Z"
            fill="black"
            data-sentry-element="path"
            data-sentry-source-file="LogoFull.tsx"
          ></path>
          <path
            d="M62.2042 24.74L60.8922 22.798L60.3682 20.436V17.338L60.8922 14.976L62.2042 13.034L64.8542 11.748H67.6362L70.2862 13.034L71.5982 14.976L72.1222 17.338V20.436L71.5982 22.798L70.2862 24.74L67.6362 26H64.8542L62.2042 24.74ZM67.2702 23.874L68.8182 23.112L69.6062 21.93L69.9482 20.172V17.548L69.6062 15.842L68.7922 14.688L67.2702 13.9H65.2222L63.6482 14.688L62.8862 15.842L62.5442 17.548V20.172L62.8862 21.93L63.6742 23.112L65.2222 23.874H67.2702Z"
            fill="black"
            data-sentry-element="path"
            data-sentry-source-file="LogoFull.tsx"
          ></path>
          <path
            d="M78.1616 25.318L76.7176 23.244V14.452H74.2236V12.3H76.7176V7.46997H78.8956V12.3H82.5176V14.452H78.8956V22.536L79.5516 23.48L80.4956 23.848H81.2296L83.0936 23.376L83.6456 25.45L81.2056 26.002H79.9976L78.1596 25.32L78.1616 25.318Z"
            fill="black"
            data-sentry-element="path"
            data-sentry-source-file="LogoFull.tsx"
          ></path>
          <path
            d="M88.5034 25.318L87.0594 23.244V14.452H84.5654V12.3H87.0594V7.46997H89.2374V12.3H92.8594V14.452H89.2374V22.536L89.8934 23.48L90.8374 23.848H91.5714L93.4354 23.376L93.9874 25.45L91.5474 26.002H90.3394L88.5014 25.32L88.5034 25.318Z"
            fill="black"
            data-sentry-element="path"
            data-sentry-source-file="LogoFull.tsx"
          ></path>
          <path
            d="M98.4234 25.448L97.1634 24.714L95.6934 22.588V21.066L96.0614 19.282L98.0034 17.34L100.601 16.684H102.281L104.039 17.262L104.669 17.708V15.714L104.381 14.926L103.279 14.218L101.993 13.904H100.129L99.0014 14.272L98.1874 14.796L97.6894 15.294L96.1934 13.746L96.9274 13.064L98.1874 12.25L99.8674 11.752H102.307L104.197 12.224L106.139 13.536L106.847 15.32V23.272L107.871 24.742L106.113 26.002L104.853 24.112L104.249 24.794L101.835 26.002H100.207L98.4234 25.45V25.448ZM101.363 23.846L103.331 23.112L104.407 21.984L104.669 21.17V20.198L104.117 19.62L103.303 19.2L102.017 18.832H100.835L99.1034 19.304L98.0794 20.328L97.8694 21.246V21.954L98.6834 23.108L99.2874 23.476L100.521 23.844H101.361L101.363 23.846Z"
            fill="black"
            data-sentry-element="path"
            data-sentry-source-file="LogoFull.tsx"
          ></path>
          <path
            d="M118.118 7.12793H120.296V23.6379H128.8V25.7899H118.118V7.12793Z"
            fill="black"
            data-sentry-element="path"
            data-sentry-source-file="LogoFull.tsx"
          ></path>
          <path
            d="M133.236 25.448L131.976 24.714L130.506 22.588V21.066L130.874 19.282L132.816 17.34L135.414 16.684H137.094L138.852 17.262L139.482 17.708V15.714L139.194 14.926L138.092 14.218L136.806 13.904H134.942L133.814 14.272L133 14.796L132.502 15.294L131.006 13.746L131.74 13.064L133 12.25L134.68 11.752H137.12L139.01 12.224L140.952 13.536L141.66 15.32V23.272L142.684 24.742L140.926 26.002L139.666 24.112L139.062 24.794L136.648 26.002H135.02L133.236 25.45V25.448ZM136.176 23.846L138.144 23.112L139.22 21.984L139.482 21.17V20.198L138.93 19.62L138.116 19.2L136.83 18.832H135.648L133.916 19.304L132.892 20.328L132.682 21.246V21.954L133.496 23.108L134.1 23.476L135.334 23.844H136.174L136.176 23.846Z"
            fill="black"
            data-sentry-element="path"
            data-sentry-source-file="LogoFull.tsx"
          ></path>
          <path
            d="M149.166 25.422L148.432 24.95V25.79H146.254V6H148.432V13.35L149.01 12.746L150.612 11.748H152.974L155.704 13.008L157.488 15.422L158.04 17.286V20.41L157.488 22.326L155.704 24.74L153.08 26H150.744L149.17 25.422H149.166ZM152.736 23.874L154.284 23.034L155.544 21.354L155.858 20.12V17.574L155.544 16.392L154.284 14.686L152.63 13.898H150.898L149.192 15.106L148.43 16.234V22.192L148.982 22.874L149.98 23.53L151.056 23.872H152.736V23.874Z"
            fill="black"
            data-sentry-element="path"
            data-sentry-source-file="LogoFull.tsx"
          ></path>
          <path
            d="M162.632 25.422L161.398 24.45L160.794 23.768L162.316 22.298L162.814 22.902L163.522 23.454L165.15 23.874H166.908L168.01 23.56L168.666 23.088L169.086 22.432V21.434L168.64 20.672L167.852 20.12L164.072 19.358L162.392 18.466L161.29 16.944V14.608L162.156 13.112L163.338 12.194L164.966 11.748H167.196L169.348 12.326L170.922 13.402L169.74 15.16L168.48 14.294L167.01 13.848H165.252L164.254 14.136L163.808 14.504L163.44 15.134V16.288L163.964 16.996L164.698 17.364L168.714 18.152L170.21 19.15L171.234 20.83V23.06L170.236 24.582L168.976 25.474L167.192 25.998H164.856L162.626 25.42L162.632 25.422Z"
            fill="black"
            data-sentry-element="path"
            data-sentry-source-file="LogoFull.tsx"
          ></path>
        </g>
        <defs data-sentry-element="defs" data-sentry-source-file="LogoFull.tsx">
          <clipPath
            id="clip0_4_8941"
            data-sentry-element="clipPath"
            data-sentry-source-file="LogoFull.tsx"
          >
            <rect
              width="171.24"
              height="32"
              fill="white"
              data-sentry-element="rect"
              data-sentry-source-file="LogoFull.tsx"
            ></rect>
          </clipPath>
        </defs>
      </svg>
    </svg>
  );
};
